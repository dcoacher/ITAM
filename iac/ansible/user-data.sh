...
if ! kubectl cluster-info &> /dev/null; then
    ...
    echo "Please ensure:"
    echo "  1. Cluster is initialized: ls -la /etc/kubernetes/admin.conf"
    echo "  2. Kubeconfig is set: ls -la ~/.kube/config"
    echo "  3. API server is running: kubectl get nodes"
    exit 1
fi
...
echo "Step 1: Deploying NFS PersistentVolume..."
if [ ! -f "nfs-pv.yaml" ]; then
    echo "Error: nfs-pv.yaml not found in current directory"
    exit 1
fi
...
if command -v helm &> /dev/null; then
    ...
    kubectl apply -f templates/deployment.yaml || true
    kubectl apply -f templates/service.yaml || true
fi
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
    ...
    if [ "$TOTAL_PODS" -gt 0 ]; then
        ...
        echo "Waiting... Running: $RUNNING_PODS/$TOTAL_PODS pods ($ELAPSED/$MAX_WAIT seconds)"
    else
...
echo ""
echo "PVC:"
kubectl get pvc itam-app-pvc 2>/dev/null || echo "Could not retrieve PVC"
