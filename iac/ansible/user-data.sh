...
apt update
apt install -y ansible python3 python3-pip python3-venv git curl
...
- name: Initialize control-plane node
  ...
  tasks:
    ...
    - shell: |
        timeout=180;elapsed=0;node_count=$(kubectl get nodes --no-headers 2>/dev/null | wc -l)
        while [ $elapsed -lt $timeout ]; do
          ...
          [ "$flannel_running" -ge "$node_count" ] && [ "$flannel_ready" -ge "$node_count" ] && exit 0
          sleep 5;elapsed=$((elapsed+5))
        done
        exit 0
...
if ! kubectl cluster-info &> /dev/null; then echo "Error: Cannot connect to API server"; exit 1; fi
[ $(kubectl get nodes --no-headers 2>/dev/null | wc -l) -eq 0 ] && echo "Error: No nodes found" && exit 1
[ ! -f "nfs-pv.yaml" ] && echo "Error: nfs-pv.yaml not found" && exit 1
kubectl apply -f nfs-pv.yaml && sleep 2
if command -v helm &> /dev/null; then
  [ ! -f "values.yaml" ] && echo "Error: values.yaml not found" && exit 1
  helm upgrade --install itam-app . --values values.yaml --timeout 5m --wait=false || true
else
  [ ! -d "templates" ] && echo "Error: templates directory not found" && exit 1
  kubectl apply -f templates/pvc.yaml templates/deployment.yaml templates/service.yaml || true
fi
MAX_WAIT=300;ELAPSED=0;READY=false
while [ $ELAPSED -lt $MAX_WAIT ]; do
  ...
  [ "$TOTAL" -gt 0 ] && kubectl wait --for=condition=available deployment/itam-app --timeout=10s &>/dev/null && READY=true && break
  echo "Waiting... Running: $RUNNING/$TOTAL ($ELAPSED/$MAX_WAIT)"
  sleep 10;ELAPSED=$((ELAPSED+10))
done
echo "";echo "Deployment Status:";echo ""
kubectl get pods -l app=itam-app 2>/dev/null || echo "Could not retrieve pods"
echo "";kubectl get svc itam-app 2>/dev/null || echo "Could not retrieve service"
echo "";kubectl get pvc itam-app-pvc 2>/dev/null || echo "Could not retrieve PVC"
[ "$READY" = true ] && echo "" && echo "Deployment completed successfully" || (echo "";echo "Deployment may still be in progress")
echo "";echo "To access: http://<node-ip>:31415"
EOF
