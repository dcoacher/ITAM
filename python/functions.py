# Functions file contains functions for reusable logic
# Importing necessary modules
import getpass # Importing Getpass module for password input data masking (required for functions.login_screen(), but Temporarily Disabled)
import storage # Importing Storage module
...
def _initial_counter(db): # Initial Counter Function
    if not db:
        return 1
...
def main_menu_modify_item():    # Modify Item Main Menu Function
    ...
    if item_id in items_db: # In case the provided Item ID by the User exist in the Item Database
        ...
        if change_type.lower() == "mfg":    # In case the User's choise is "mfg"
            ...
            storage.save_items(items_db)    # Save the items database to the file
...
def main_menu_assign_item():    # Assign Item Main Menu Function
    while True:
        ...
        if item_id not in items_db or user_id not in users_db: # In case the Item ID or/and User ID are not exists in the Databases
